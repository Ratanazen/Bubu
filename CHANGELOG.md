...
### Added
- **Developer App Panel**: Introduced `@bubu/developer-engine` for local toolchain detection (gcc, node, python, etc.), project scanning (CMake, npm, Cargo), and integrated build execution.
- **Developer Panel UI**: Added a dedicated Developer Dashboard inside the Control Center to monitor builds and system resources.
- **Low-End Mode**: Developer engine processes are throttled to ensure older computers do not freeze during compilation.
...
- **Unix Domain Sockets**: Bubu Desktop Daemon now hosts a secure UNIX socket to handle native IPC correctly across Wayland compositors.
